...
FUZZ_TEST(MachOTest, MachOFuzzWithCorpus)
    .WithSeeds(fuzztest::ReadFilesFromDirectory(absl::StrCat(std::getenv("TEST_SRCDIR"),
                                                             kCorpusPath)));

FUZZ_TEST(MachOTest, MachOFuzzWithKnownSize).WithDomains(fuzztest::Arbitrary<std::vector<char>>());
