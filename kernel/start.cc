...
darwin::DarwinKit* rootkit = nullptr;

darwin::DarwinKit* mac_rootkit_get_rootkit() {
    if (rootkit)
        return rootkit;
...
kern_return_t mac_rootkit_start(IOKernelDarwinKitService* service, Kernel* kernel, Kext** kext) {
    ...
    rootkit = new darwin::DarwinKit(kernel);
    ...
    *kext = rootkit->getKextByIdentifier("com.YungRaj.DarwinKit");
    ...
    if (!*kext) {
        DARWIN_RK_LOG("MacRK::mac_rootkit_start() cannot find com.YungRaj.DarwinKit kext!\n");
    } else {
        DARWIN_RK_LOG("MacRK::mac_rootkit_start() found com.YungRaj.DarwinKit kext!\n");
    }
