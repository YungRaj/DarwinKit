...
UInt64 debug::ReadUleb128(UInt8* p, UInt8* end) {
    ...
    do {
        if (p == end) {
            DARWIN_RK_LOG("malformed uleb128\n");
        ...
        if (bit > 63) {
            DARWIN_RK_LOG("uleb128 too big for uint64\n");
...
Int64 debug::ReadSleb128(UInt8* p, UInt8* end) {
    ...
    do {
        if (p == end) {
            DARWIN_RK_LOG("malformed sleb128\n");
...
DIE<T>::DIE(Dwarf<T>* dwarf, UInt64 code, char* name, enum DW_TAG tag,
...
void Dwarf<T>::parseDebugAbbrev() {
    ...
    Sect debug_info = __debug_info;
    Sect debug_abbrev = __debug_abbrev;
    Sect debug_str = __debug_str;
    ...
    while (debug_abbrev_offset < debug_abbrev->getSize() - sizeof(UInt32)) {
        if (code == 0 && static_cast<UInt32>(tag) == 0) {
            ...
            if (static_cast<int>(attr) == 0 && static_cast<int>(form) == 0) {
                ...
                DARWIN_RK_LOG("\tDW_AT = %s 0x%x DW_FORM = %s\n", DWAttrToString(attr),
...
void Dwarf<T>::parseDebugInfo() {
    ...
    struct CompilationUnit<T>* compilationUnit = nullptr;
    ...
    while (debug_info_offset < debug_info->getSize() - sizeof(UInt32)) {
        ...
        DwarfDIE<T>* parent = stack.size() > 0 ? stack.at(stack.size() - 1) : nullptr;
        ...
        for (int i = 0; i < attributes_count; i++) {
            ...
            DARWIN_RK_LOG("\tDW_AT = %s value = 0x%llx\n", DWAttrToString(attr), value);
        }
...
void Dwarf<T>::parseDebugLines() {
    ...
    while (debug_line_offset < debug_line->getSize()) {
        LineTable<T>* lineTable = new LineTable<T>(binary, this);
        ...
        while ((debug_line_begin + debug_line_offset) < prologue_end) {
            if (source_file_names) {
                ...
                DARWIN_RK_LOG("Source File Name: %s\n", source_file_name);
                ...
                DARWIN_RK_LOG("Include Directory: %s\n", include_directory);
        ...
        DARWIN_RK_LOG("%-20s %-6s %-6s %-6s %-4s %-13s %-13s\n", "Address", "Line", "Column", "File",
        ...
        while ((debug_line_begin + debug_line_offset) < end) {
            ...
            if (op == 0) {
                ...
                case DW_LNE::end_sequence:;
                    {
                        ...
                        DARWIN_RK_LOG("0x%-20llx %-6lld %-8lld %-6u %-4u %-13u %-13s\n",
                ...
                case DW_LNS::advance_line:;
                    {
                        Int64 line = debug::ReadSleb128(debug_line_begin + debug_line_offset,
                                                        debug_line_end, &debug_line_offset);
...
void Dwarf<T>::parseDebugLocations() {
    ...
    while (debug_loc_offset < debug_loc->getSize()) {
        ...
        if (value0 != 0 || value1 != 0) {
            ...
            DARWIN_RK_LOG("\t(0x%016llx, 0x%016llx) ", value0, value1);
...
void Dwarf<T>::parseDebugRanges() {
    ...
    Sect debug_ranges = __debug_ranges;
...
void Dwarf<T>::parseDebugAddressRanges() {
    ...
    Sect debug_aranges = __debug_aranges;
    ...
    while (debug_aranges_offset < debug_aranges->getSize()) {
        ...
        DARWIN_RK_LOG("Address Range Header: length = 0x%08x, version = 0x%04x, cu_offset = 0x%08x, "
        ...
        while (offset < debug_aranges_offset + length) {
            ...
            if (value0 != 0 || value1 != 0) {
                ...
                DARWIN_RK_LOG("(0x%016llx, 0x%016llx)\n", address, address + size);
